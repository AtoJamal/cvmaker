PROMPTS = {
    'en': {
        'welcome':"Welcome to the Kelem CV Producer bot!",
        'welcome_new': "<pre>Welcome to the Kelem CV Producer bot!</pre> \nPlease enter your first name:",
        'choose_option':"Please choose what you would like to do next",
        'welcome_back': "Welcome back! You already have a profile. What would you like to do next?",
        'select_language': "Please select your preferred language:\nእባክዎ የሚፈልጉትን ቋንቋ ይምረጡ፡",
        'update_profile': "Update Profile",
        'new_cv': "Create New CV",
        'edit_section': "Which section would you like to update?",
        'personal_info': "Personal Info",
        'contact_info': "Contact Info",
        'work_experience': "Work Experience",
        'education': "Education",
        'skills': "Skills",
        'careear_objective': "Career Objective",
        'certifications': "Certifications",
        'projects': "Projects",
        'languages': "Languages",
        'other_activities': "Other Activities",
        'first_name': "Please enter your first name:",
        'middle_name': "Great! Now please enter your middle name (if any):",
        'last_name': "Now please enter your last name:",
        'phone_number': "Please enter your phone number (e.g., +251911223344):",
        'email_address': "Please enter your email address:",
        'linkedin_profile': "Please enter your LinkedIn profile URL (or type 'skip', if none):",
        'city': "What city do you currently live in?",
        'country': "Please enter your country:",
        'job_title': "Work Experience\nYour most recent job title?",
        'company_name': "Please enter the **company name** for this job(Ethio Telecom)",
        'work_location': "Where was this job located? (e.g., 'Addis Ababa, Ethiopia')",
        'work_description': "Describe your role, in this job and the duration(e.g., 'Managed team , Jan 2022 - Dec 2023, or type 'skip' if none):",
        'add_another_work': "Work experience added. Would you like to add another position?\nPlease select an option below:",
        'add_another': "Add Another",
        'continue': "Next",
        'degree_name': "What's your degree name? (e.g., Bachelor of Science in Computer Science)",
        'institution_name': "University / Institution name for your degree (e.g., Mekelle University).",
        'gpa': "What was your GPA for this degree? and university exit exam(e.g., '3.5/4.0' & '60/100', or type 'skip')",
        'edu_description': "Start and end dates for this degree(e.g., 'Sept 2018 - June 2022').",
        'achievements_honors': "Please list any achievements or honors (e.g., type 'skip' ):",
        'add_another_edu': "Degree added. Would you like to add another degree entry?\nPlease select an option below:",
        'skill_name': "What's a key skill you gained from your degree? This could be a technical skill, a research method (e.g., 'Graphic Design' , 'Data Analysis')",
        'skill_proficiency': "Please enter your proficiency level for this skill (e.g., 'Beginner', 'Intermediate', 'Advanced'):",
        'add_another_skill': "Skill added. Would you like to add another skill?\nPlease select an option below:",
        'career_summary': "Please tell us the name of your high school and its location (e.g., \n'Menelik II Secondary School, Addis Ababa, Ethiopia'.",
        'certificate_name': "Do you have Any certifications or awards? (or, type 'skip' if none):",
        'issuer': "From which organization did you receive this certification? (e.g., 'Ethio-telecom Training Center')",
        'add_another_cert': "Certification added. Would you like to add another certification?\nPlease select an option below:",
        'project_title': "Your final year senior university project title? ",
        'project_description': "Describe your contributions, methods, outcomes, and dates in this project (or type 'skip' if none):",
        'project_link': "Please provide a link to the project/research (type 'skip' if none):",
        'add_another_project': "Project added. Would you like to add another project?\nPlease select an option below:",
        'language_name': "Please enter a language you speak, one at a time (e.g., 'Amharic','English'):",
        'language_proficiency': "Please enter your proficiency level for this language (e.g., 'Fluent', 'Native', 'Intermediate'):",
        'add_another_language': "Language added. Would you like to add another language?\nPlease select an option below:",
        'activities': "internship title and Reference  (or, type 'skip' if none):",
        'summary_header': "Here's a summary of your information:\n\n",
        'summary_name': "Name",
        'summary_contact': "Contact",
        'summary_location': "Location",
        'summary_availability': "Availability",
        'summary_work': "Work Experience",
        'summary_responsibilities': "Responsibilities",
        'summary_education': "Education",
        'summary_gpa': "GPA & Exit Exam",
        'summary_edu_description': "Duration",
        'summary_achievements': "Achievements/Honors",
        'summary_skills': "Skills",
        'summary_proficiency': "Proficiency",
        'summary_certifications': "Certifications/Awards",
        'summary_projects': "Projects",
        'summary_project_link': "Link",
        'summary_languages': "Languages",
        'confirm': "✅ Confirm",
        'edit': "✏️ Edit",
        'payment_instructions': "Please pay {price} Birr to:\n\nBank: Commercial Bank of Ethiopia\nAccount: <code>1000649561382</code> [click to copy] \nName: Jemal Hussen Hassen\n\nTelebirr: <code>+251906365418</code> \nName: Jemal \n\nAfter payment, upload a screenshot of the confirmation.",
        'payment_confirmation': "Thank you! Your payment is being processed. We will notify you once it's verified. Please come back later.",
        'cancel_message': "Operation cancelled. Type /start to begin again.",
        'help_message': "Use /start to create or update your CV profile.\nUse /cancel to stop the current operation.\nUse /payment to reupload a new payment screenshot\nChat : @atomimore",
        'error_message': "An error occurred. Please try again or contact support.",
        'profile_image_prompt': "Please upload your profile image ( max 5 MB or Type 'skip' )",
        'profile_image_success': "Profile image uploaded successfully. Proceed to professional information?",
        'invalid_file_type': "Invalid file type. Please upload a JPG, JPEG, PNG file",
        'file_too_large': "File too large. Please upload an image or file under 5 MB.",
        'profile_image_skip': "Profile image skipped. Proceed to professional information?",
        'continue_professional': "Continue to Professional Info",
        'payment_instructions': "Please pay {price} Birr to:\n\nBank: Commercial Bank of Ethiopia\nAccount: <code>1000649561382</code> [click to copy] \nName: Jemal Hussen Hassen\n\nTelebirr: <code>+251906365418</code> \nName: Jemal\n\nAfter payment, upload a screenshot of the confirmation.",
        'payment_screenshot_success': "Payment screenshot uploaded successfully. Awaiting verification.",
        'payment_verified': "✅ Your payment has been verified! Your CV is being processed.",
        'payment_rejected': "❌ Your payment was rejected: {reason}. Please reupload a valid screenshot with /payment.",
        'payment_approved': "Your payment has been approved! Your CV is being processed and will be delivered soon.",
        'reject_reason_prompt': "Please provide the reason for rejecting the payment.",
        'editing_started':"editing started",
        'work_experience_skip_hint':"Type 'skip' if you don't have any work experience.",
        'job_title_with_skip':"Work Experience\nYour most recent job title?(or type 'skip'):",
        'payment_retry_instructions':"Your previous payment was rejected. Please upload a new payment screenshot.\n\nBank: Commercial Bank of Ethiopia\nAccount: <code>1000649561382</code> [click to copy] \nName: Jemal Hussen Hassen\n\nTelebirr: <code>+251906365418</code> \nName: Jemal\n\n",
        'no_rejected_payment':"You don't have a rejected payment. Use /start to create a new CV order.",
        'payment_retry_success':"Payment screenshot uploaded successfully! Please wait for admin verification.",
        'tutorial_message': '📹 Here\'s a helpful video guide on how to fill out your CV properly:',
        'choose_action': '👋 Welcome! Please choose what you\'d like to do:',
        'create_cv': '📝 Create CV',
        'samples': '📁 View Samples',
        'guide_video': '📹 Guide Video',
        'create_new_cv': '🆕 Create New CV',
        'sending_samples': '📁 Here are some sample CVs for your reference:',
        'samples_sent': '✅ Sample CVs have been sent above. You can use these as reference for creating your own CV.',
        'payment_instructions': "Please pay {price} Birr to:\n\nBank: Commercial Bank of Ethiopia\nAccount: <code>1000649561382</code> [click to copy] \nName: Jemal Hussen Hassen\n\nTelebirr: <code>+251906365418</code> \nName: Jemal\n\nAfter payment, upload a screenshot of the confirmation.",
        # Plan selection UI
        'select_plan_prompt': "Please choose a plan:",
        'plan_premium_title': "---------- Premium ----------\nPrice : 350 Birr\nDelivered within one day\nQuality the same\nFeedback count one\n",
        'plan_premium_price': "350",
        'plan_premium_select': "Select Premium",
        'payment_instruction_premium': "Please pay 350 Birr to:\n\nBank: Commercial Bank of Ethiopia\nAccount: <code>1000649561382</code>\nName: Jemal Hussen Hassen\n\nTelebirr: <code>+251906365418</code> \nName: Jemal\n\nAfter payment, click Upload screenshot and send the confirmation.",
        'plan_popular_title': "---------- Popular ----------\nPrice : 199 Birr\nDelivered within two days\nQuality the same\nFeedback count one\n",
        'plan_popular_price': "199",
        'plan_popular_select': "Select Popular",
        'payment_instruction_popular': "Please pay 199 Birr to:\n\nBank: Commercial Bank of Ethiopia\nAccount: <code>1000649561382</code>\nName: Jemal Hussen Hassen\n\nTelebirr: <code>+251906365418</code> \nName: Jemal\n\nAfter payment, click Upload screenshot and send the confirmation.",
        'plan_standard_title': "---------- Standard ----------\nPrice : 90 Birr\nDelivered within seven days\nQuality the same \nFeedback count one\n",
        'plan_standard_price': "90",
        'plan_standard_select': "Select Standard",
        'payment_instruction_standard': "Please pay 90 Birr to:\n\nBank: Commercial Bank of Ethiopia\nAccount: <code>1000649561382</code>\nName: Jemal Hussen Hassen\n\nTelebirr: <code>+251906365418</code> \nName: Jemal\n\nAfter payment, click Upload screenshot and send the confirmation.",
        'upload_screenshot_button': "Upload screenshot",
        'payment_screenshot_upload' : 'Please upload your payment screenshot now (photo or PDF).',
        # ...existing keys...

    },
    'am': {
        'welcome':"ወደ ቀለም ሲቪ ፕሮዲውሰር ቦት እንኳን በደህና መጡ!",
        'welcome_new': "ወደ ቀለም ሲቪ ፕሮዲውሰር ቦት እንኳን በደህና መጡ! የፕሮፌሽናል ሲቪዎን እንፍጠር።\n\nእባክዎ የመጀመሪያ ስምዎን ያስገቡ፡",
        'welcome_back': "እንኳን ተመልሰው መጡ! ቀድሞ ፕሮፋይል አለዎት። መረጃዎን ማዘመን ወይም አዲስ ሲቪ መፍጠር ይፈልጋሉ?",
        'select_language': "እባክዎ የሚፈልጉትን ቋንቋ ይምረጡ፡\nPlease select your preferred language:",
        'update_profile': "ፕሮፋይል አዘምን",
        'choose_option': "እባክዎ የሚፈልጉትን አማራጭ ይምረጡ፡",
        'new_cv': "አዲስ ሲቪ ፍጠር",
        'edit_section': "የትኛውን ክፍል ማዘመን ይፈልጋሉ?",
        'personal_info': "የግል መረጃ",
        'contact_info': "የእውቂያ መረጃ",
        'work_experience': "የሥራ ልምድ",
        'education': "ትምህርት",
        'skills': "ችሎታዎች",
        'career_objective': "የሙያ ግብ",
        'certifications': "ሰርቲፊኬቶች/ሽልማቶች",
        'projects': "ፕሮጀክቶች",
        'languages': "ቋንቋዎች",
        'other_activities': "ሌሎች እንቅስቃሴዎች",
        'first_name': "እባክዎ የመጀመሪያ ስምዎን ያስገቡ፡",
        'middle_name': "በጣም ጥሩ! አሁን የአባት ስምዎን (ካለ) ያስገቡ፡",
        'last_name': "አሁን የአያት ስምዎን ያስገቡ፡",
        'phone_number': "አሁን የእውቂያ መረጃዎን እንሰብስብ።\nእባክዎ የስልክ ቁጥርዎን ያስገቡ (ለምሳሌ፡ +251911223344):",
        'email_address': "እባክዎ የኢሜይል አድራሻዎን ያስገቡ፡",
        'linkedin_profile': "እባክዎ የሊንክዲን ፕሮፋይል ዩአርኤልዎን ያስገቡ (ካለ፣ ወይም 'skip' ይፃፉ):",
        'city': "እባክዎ የሚኖሩበትን ከተማ ያስገቡ፡",
        'country': "እባክዎ አገርዎን ያስገቡ፡",
        'job_title': "የስራ ልምድ\nየቅርብ ጊዜ የስራ መደብዎ ምንድን ነው?",
        'company_name': "እባክዎ ለዚህ የስራ መደብ የድርጅቱን ስም ያስገቡ",
        'work_location': "ይህ ስራ የት ነበር የሚገኘው?(ለምሳሌ፡ ከተማ፣ አገር):",
        'work_description': "በዚህ ሥራ ውስጥ ያለዎትን ሚና እና የሥራውን ቆይታ ይግለጹ (e.g., 'Managed team , Jan 2022 - Dec 2023, or type 'skip' if none):",
        'add_another_work': "የሥራ ልምድ ታክሏል። ሌላ ቦታ መጨመር ይፈልጋሉ?\nእባክዎ ከታች አማራጭ ይምረጡ፡",
        'add_another': "ሌላ ጨምር",
        'continue': "ቀጥል",
        'degree_name': "የዲግሪዎ ስም ምንድን ነው? (ለምሳሌ፦ የኮምፒውተር ሳይንስ ባችለር ኦፍ ሳይንስ):",
        'institution_name': "እባክዎ ይህንን ዲግሪ ያገኙበትን የዩኒቨርሲቲ ወይም የተቋም ስም ያስገቡ(e.g.,  'መቀሌ ዩኒቨርሲቲ'):",
        'gpa': "ለዚህ ዲግሪ የነበረው GPA ስንት ነበር? (ለምሳሌ፦ '3.5/4.0'፣ ወይም 'skip' ብለው ይጻፉ)",
        'edu_description': "የትምህርት ማስረጃዎን የጀመሩበት እና ያጠናቀቁበት ቀን(e.g., 'Sept 2018 - June 2022')",
        'achievements_honors': "እባክዎ ማንኛውንም ስኬቶች ወይም ክብር ይዘርዝሩ ('skip' ይፃፉ ከሌለ):",
        'add_another_edu': "ዲግሪ ታክሏል። ሌላ ዲግሪ መግቢያ መጨመር ይፈልጋሉ?\nእባክዎ ከታች አማራጭ ይምረጡ፡",
        'skill_name': "ከዲግሪዎ ያገኙት ቁልፍ ክህሎት ምንድን ነው? ይህ ቴክኒካዊ ክህሎት፣ የምርምር ዘዴ (e.g., 'Graphic Design' , 'Data Analysis')",
        'skill_proficiency': "እባክዎ ለዚህ ችሎታ የብቃት ደረጃዎን ያስገቡ (ለምሳሌ፡ 'ጀማሪ'፣ 'መካከለኛ'፣ 'ከፍተኛ'):",
        'add_another_skill': "ችሎታ ታክሏል። ሌላ ችሎታ መጨመር ይፈልጋሉ?\nእባክዎ ከታች አማራጭ ይምረጡ፡",
        'career_summary': "እባክዎ የሁለተኛ ደረጃ ትምህርት ቤትዎን ስም እና የሚገኝበትን ቦታ ይንገሩን (e.g., \n'Menelik II Secondary School, Addis Ababa, Ethiopia'.",
        'certificate_name': "ማንኛውም የምስክር ወረቀቶች ወይም ሽልማቶች አሉዎት?",
        'issuer': "ይህን የምስክር ወረቀት ከየትኛው ድርጅት አገኙ? : (e.g., 'Ethio-telecom Training Center')",
        'add_another_cert': "ሰርቲፊኬት ታክሏል። ሌላ ሰርቲፊኬት መጨመር ይፈልጋሉ?\nእባክዎ ከታች አማራጭ ይምረጡ፡",
        'project_title': "የመጨረሻ ዓመት የዩኒቨርሲቲ ፕሮጀክትዎ ርዕስ ምንድን ነው?  ",
        'project_description': "ያበረከቱትን አስተዋጽኦ፣ ዘዴዎች፣ ውጤቶች እና ቀኖች ይግለጹ ለዚህ ፕሮጀክትዎ (or type 'skip' if none):",
        'project_link': "እባክዎ የሪሰርቹን/የፕሮጀክቱን አገናኝ(link) ያቅርቡ (e.g., GitHub repository, live demo, or type 'skip' if none):",
        'add_another_project': "ፕሮጀክት/ሪሰርች ታክሏል። ሌላ ፕሮጀክት/ሪሰርች መጨመር ይፈልጋሉ?\nእባክዎ ከታች አማራጭ ይምረጡ፡",
        'language_name': "እባክዎ የሚናገሩትን ቋንቋ ያስገቡ (e.g., 'Amahric', 'English'):",
        'language_proficiency': "እባክዎ ለዚህ ቋንቋ የብቃት ደረጃዎን ያስገቡ (ለምሳሌ፡ 'Fluent'፣ 'Native'፣ 'Intermediate'):",
        'add_another_language': "ቋንቋ ታክሏል። ሌላ ቋንቋ መጨመር ይፈልጋሉ?\nእባክዎ ከታች አማራጭ ይምረጡ፡",
        'activities': "የኢንተርንሺፕዎ ርዕስ ምን ነበር? (ወይም 'skip' ብለው ይጻፉ ከሌለ):",
        'summary_header': "የመረጃዎ ማጠቃለያ ይኸው፡\n\n",
        'summary_name': "ስም",
        'summary_contact': "እውቂያ",
        'summary_location': "መገኛ ቦታ",
        'summary_availability': "የሚገኝነት",
        'summary_work': "የሥራ ልምድ",
        'summary_responsibilities': "ኃላፊነቶች",
        'summary_education': "ትምህርት",
        'summary_gpa': "ጂፒኤ",
        'summary_edu_description': "መግለጫ",
        'summary_achievements': "ስኬቶች/ክብር",
        'summary_skills': "ችሎታዎች",
        'summary_proficiency': "ብቃት",
        'summary_certifications': "ሰርቲፊኬቶች/ሽልማቶች",
        'summary_projects': "ፕሮጀክቶች",
        'summary_project_link': "አገናኝ",
        'summary_languages': "ቋንቋዎች",
        'confirm': "✅ አረጋግጥ",
        'edit': "✏️ አርም",
        'payment_instructions': "የክፍያ መመሪያዎች\n\nእባክዎ {price} ብር ወደሚከተለው የባንክ ሂሳብ ያስገቡ፡\n\nባንክ: የኢትዮጵያ ንግድ ባንክ\n <code>1000649561382</code> [click to copy] \nስም: ጀማል ሁሴን ሀሰን\n\nቴሌብር: <code>+251906365418</code> \nስም: ጀማል\n\nክፍያ ከፈጸሙ በኋላ፣ የክፍያ ማረጋገጫውን ቅጽበታዊ ገጽ እይታ (screenshot) ይላኩልን።",
        'payment_confirmation': "እናመሰግናለን! ክፍያዎ በሂደት ላይ ነው። ከተረጋገጠ በኋላ እናሳውቅዎታለን። እባክዎ ቆይተው ይመለሱ።",
        'cancel_message': "ክወናው ተሰርዟል። እንደገና ለመጀመር /start ይፃፉ።",
        'help_message': "ሲቪ ፕሮፋይልዎን ለመፍጠር ወይም ለማዘመን /start ይጠቀሙ።\nክወናውን ለማቆም /cancel ይጠቀሙ።\nአዲስ ክፍያ እንደገና ለመጫን ይህንን/payment ይጠቀሙ።\nቻት፡ @atomimore",
        'error_message': "ስህተት ተከስቷል። እባክዎ እንደገና ይሞክሩ ወይም ድጋፍ ያግኙ።",
        'profile_image_prompt': "እባክዎ የፕሮፋይል ምስልዎን (ከፍተኛ 5 ሜባ) ይስቀሉ። ያለ ምስል ለመቀጠል 'skip' ይፃፉ። ",
        'profile_image_success': "የፕሮፋይል ምስል በተሳካ ሁኔታ ተሰቅሏል። ወደ ሙያዊ መረጃ መቀጠል?",
        'invalid_file_type': "የተሳሳተ የፋይል አይነት። እባክዎ JPG, JPEG, PNG ፋይል ይስቀሉ።",
        'file_too_large': "ፋይሉ በጣም ትልቅ ነው። እባክዎ ከ5 ሜባ በታች ያለ ምስል ወይም ፋይል ይስቀሉ።",
        'profile_image_skip': "የፕሮፋይል ምስል ተዘልሏል። ወደ ሙያዊ መረጃ መቀጠል?",
        'continue_professional': "ወደ ሙያዊ መረጃ ቀጥል",
        'payment_instructions': "የክፍያ መመሪያዎች\n\nእባክዎ {price} ብር ወደሚከተለው የባንክ ሂሳብ ያስገቡ፡\n\nባንክ: የኢትዮጵያ ንግድ ባንክ\n <code>1000649561382</code> [click to copy] \nስም: ጀማል ሁሴን ሀሰን\n\nቴሌብር: <code>+251906365418</code> \nስም: ጀማል\n\nክፍያ ከፈጸሙ በኋላ፣ የክፍያ ማረጋገጫውን ቅጽበታዊ ገጽ እይታ (screenshot) ይላኩልን።",
        'payment_screenshot_success': "የክፍያ ማረጋገጫ ፎቶ በተሳካ ሁኔታ ተሰቅሏል። ማረጋገጫ በመጠበቅ ላይ።",
        'payment_verified': "✅ ክፍያዎ ተረጋግጧል! ሲቪዎ በመዘጋጀት ላይ ነው።",
        'payment_rejected': "❌ ክፍያዎ ተቀባይነት አላገኘም፡ {reason}። እባክዎ ህጋዊ screenshot እንደገና ይስቀሉ። /payment ይጠቀሙ።",
        'payment_approved': "ክፍያዎ ተፈቅዷል! ሲቪዎ በመዘጋጀት ላይ ነው እና በቅርቡ ይደርሰዎታል።",
        'reject_reason_prompt': "እባክዎ ክፍያውን ለመከልከል ምክንያቱን ያቅርቡ።",
        'work_experience_skip_hint':"የስራ ልምድ\nየቅርብ ጊዜ የስራ መደብዎ ምንድን ነው? (ከሌልዎት 'skip' ይፃፉ።)",
        'job_title_with_skip': "እባክዎ የስራ መደብዎን ያስገቡ (ወይም 'skip' ብለው ይፃፉ):",
        'payment_retry_instructions':"የቀደመው ክፍያዎ ተቋርጧል። እባክዎን አዲስ የክፍያ ቅጽበተ-ገጽ ያስገቡ\n\nባንክ: የኢትዮጵያ ንግድ ባንክ\n <code>1000649561382</code> \nስም: ጀማል ሁሴን ሀሰን\n\nቴሌብር: <code>+251906365418</code> \nስም: ጀማል\n\n",
        'no_rejected_payment':"የተቋረጠ ክፍያ የለዎትም። አዲስ የሲቪ ትዕዛዝ ለመፍጠር /start ይጠቀሙ።",
        'payment_retry_success':"የክፍያ ቅጽበተ-ገጽ በተሳካ ሁኔታ ተሰቅሏል! እባክዎን ለአስተዳዳሪ ማረጋገጫ ይጠብቁ።",
        'tutorial_message': '📹 የሲቪ ማቀነባበር መመሪያ ቪዲዮ:',
        'choose_action': '👋 እንኳን በደህና መጡ! ምን ማድረግ እንደሚፈልጉ ይምረጡ:',
        'create_cv': '📝 ሲቪ ይፍጠሩ',
        'samples': '📁 ናሙና ይመልከቱ',
        'guide_video': '📹 መመሪያ ቪዲዮ',
        'create_new_cv': '🆕 አዲስ ሲቪ ይፍጠሩ',
        'sending_samples': '📁 የሲቪ ናሙናዎች:',
        'samples_sent': '✅ የሲቪ ናሙናዎች ተልከዋል። እነዚህን እንደ ማጣቀሻ ተጠቅመው የራስዎን ሲቪ መፍጠር ይችላሉ።',
        'payment_instructions': "የክፍያ መመሪያዎች\n\nእባክዎ {price} ብር ወደሚከተለው የባንክ ሂሳብ ያስገቡ፡\n\nባንክ: የኢትዮጵያ ንግድ ባንክ\n <code>1000649561382</code> [click to copy] \nስም: ጀማል ሁሴን ሀሰን\n\nቴሌብር: <code>+251906365418</code> \nስም: ጀማል\n\nክፍያ ከፈጸሙ በኋላ፣ የክፍያ ማረጋገጫውን ቅጽበታዊ ገጽ እይታ (screenshot) ይላኩልን።",
        # Plan selection UI (Amharic)
        'select_plan_prompt': "እባክዎ እቅድ ይምረጡ፡",
        'plan_premium_title': "---------- ፕሪሚየም ----------\nዋጋ: 350 ብር\nበአንድ ቀን ውስጥ ይደርሳል\nእኩል ጥራት\nአንድ ጊዜ አስተያየት",
        'plan_premium_price': "350",
        'plan_premium_select': "ፕሪሚየም ይምረጡ",
        'payment_instruction_premium': "እባክዎ 350 ብር ይከፍሉ፡\n\nባንክ: የኢትዮጵያ ንግድ ባንክ\n<code>1000649561382</code>\nስም: ጀማል ሁሴን ሀሰን\n\nቴሌብር: <code>+251906365418</code> \nስም: ጀማል\n\nክፍያ ከፈጸሙ በኋላ፣ ስክሪንሾት ይስቀሉ።",
        'plan_popular_title': "---------- ታዋቂ ----------\nዋጋ: 199 ብር\nበሁለት ቀናት ውስጥ ይደርሳል\nእኩል ጥራት\nአንድ ጊዜ አስተያየት",
        'plan_popular_price': "199",
        'plan_popular_select': "ታዋቂ ይምረጡ",
        'payment_instruction_popular': "እባክዎ 199 ብር ይከፍሉ፡\n\nባንክ: የኢትዮጵያ ንግድ ባንክ\n<code>1000649561382</code>\nስም: ጀማል ሁሴን ሀሰን\n\nቴሌብር: <code>+251906365418</code> \nስም: ጀማል\n\nክፍያ ከፈጸሙ በኋላ፣ ስክሪንሾት ይስቀሉ።",
        'plan_standard_title': "---------- መደበኛ ----------\nዋጋ: 90 ብር\nበሶስት የስራ ቀናት ውስጥ ይደርሳል\nእኩል ጥራት\nአንድ ጊዜ አስተያየት",
        'plan_standard_price': "90",
        'plan_standard_select': "መደበኛ ይምረጡ",
        'payment_instruction_standard': "እባክዎ 90 ብር ይከፍሉ፡\n\nባንክ: የኢትዮጵያ ንግድ ባንክ\n<code>1000649561382</code>\nስም: ጀማል ሁሴን ሀሰን\n\nቴሌብር: <code>+251906365418</code> \nስም: ጀማል\n\nክፍያ ከፈጸሙ በኋላ፣ ስክሪንሾት ይስቀሉ።",
        'upload_screenshot_button': "ስክሪንሾት ይስቀሉ",
        'payment_screenshot_upload' : 'እባክዎ የክፍያ ማረጋገጫውን ቅጽበታዊ ገጽ እይታ (screenshot) ይላኩልን።',
        # ...existing keys...
    }
}
